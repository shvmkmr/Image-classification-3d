...
class BrainRSNADataset(Dataset):
    def __init__(
        self, data, transform=None, target="target-label", img_type="FLAIR", is_train=True, ds_type="forgot", do_load=True
    ):
        self.target = target
        self.data = data
        self.type = img_type
        ...
        self.folder = "raw" if self.is_train else "test"
    ...
    def _prepare_biggest_images(self):
        big_image_indexes = {}
        #if (f"big_image_indexes_{self.ds_type}.pkl" in os.listdir("../Input/")) and (self.do_load) :
        if (f"big_image_indexes_{self.ds_type}.pkl" in os.listdir(config.inp_dir)) and (self.do_load) :
            ...
            big_image_indexes = joblib.load(f"{config.inp_dir}big_image_indexes_{self.ds_type}.pkl")
            #print('debug 2 done')#debug
            ...
            for row in tqdm(self.data.iterrows(), total=len(self.data)):
                case_id = str(int(row[1].sampleID)).zfill(5)
                path = f"../Input/raw/{self.folder}/{case_id}/{self.type}/*.dcm"
            ...
            joblib.dump(big_image_indexes, f"{config.inp_dir}big_image_indexes_{self.ds_type}.pkl")
            return big_image_indexes
    ...
    def load_dicom_images_3d(
        ...
        #path = f"../Input/raw/{case_id}/{self.type}/*.dcm"
        files = sorted(
            glob.glob(path),
            key=lambda var: [
                int(x) if x.isdigit() else x for x in re.findall(r"[^0-9]|[0-9]+", var)
        ...
        #print("This is debug",len(files[p1:p2]))#Debug
